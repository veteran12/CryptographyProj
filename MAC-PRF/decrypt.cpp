...
int main(int argc, char* argv[])
{
	...
	e.SetKeyWithIV(key1, sizeof(key1), ivv);
        string hashtmp;
	
	StringSource(cipher + timestamp, true,
                 new StreamTransformationFilter(e,
                                            new StringSink(hashtmp)
    ...
    finish=clock();
    cout<< "the CBC-AES encrypt running time is " << difftime(finish,start) << " ms" << endl;
